...
export const SYNONYM_MAP = {
  ...
  'co-amoxiclav': ['amoxicillin and clavulanate potassium', 'augmentin', 'amoxicillin clavulanate'],
  ...
  'artemether-lumefantrine': ['artemether and lumefantrine', 'coartem', 'lumefantrine'],
  ...
  cotrimoxazole: ['sulfamethoxazole and trimethoprim', 'bactrim', 'septrin'],
...
// ── Generic openFDA Axios Caller with Key Injection & Cache ─────────────────
...
export const fetchDrugLabel = async (drugName) => {
  ...
  for (const term of searchTerms) {
    ...
    if (data?.results?.[0]) {
      ...
      return {
        ...
        genericName: openfda.generic_name?.[0] || term,
        brandNames: openfda.brand_name || [],
        substanceNames: openfda.substance_name || [],
        activeIngredients: label.active_ingredient || openfda.substance_name || [],
  ...
  // Fallback: search general text if structured fields yielded nothing
  try {
...
export const fetchAdverseEvents = async (drugName, options = {}) => {
  ...
  const searchTerms = getSearchTerms(drugName);
  const primaryTerm = searchTerms[0] || drugName;
...
export const getComprehensiveDrugProfile = async (query, patientContext = {}) => {
  ...
  // Concurrent fetch of all key openFDA datasets
  ...
  // Fetch adverse event signals if label exists
  ...
  if (label) {
    ...
    adverseEvents = await fetchAdverseEvents(label.genericName || query, { ageGroup, sex });
  }

  // Calculate clinical alert checks
  ...
  // Compute composite regulatory trust score (0 - 100%)
  ...
  return {
    query,
    resolvedName: label?.genericName || label?.brandName || query,
    label,
    ndc,
    recalls,
    approvals,
