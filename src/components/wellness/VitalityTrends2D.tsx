...
export function VitalityTrends2D({ data }: VitalityTrends2DProps) {
  ...
  // Right Axis: Wellness rating (1 - 5)
  const getWellnessY = React.useCallback((val: number | null) => {
    if (val === null) return 0;
    // Rating 1 is at bottom, 5 is at top
    return viewHeight - paddingBottom - ((val - 1) / 4) * chartHeight;
  }, []);
  ...
  // Generate Adherence Area Path (closes the bottom of the SVG to draw a gradient area)
  ...
  const handlePointerMove = (e: React.PointerEvent<SVGSVGElement>) => {
    ...
    // Normalized client coordinates
    const clientX = e.clientX - rect.left;
    const clientY = e.clientY - rect.top;

    // Map client X to viewBox coordinates
    ...
    // Clamp tooltip Y slightly above the pointer, minimum 10px
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Right y-axis labels (Wellness Ratings 1-5) */}
        {[1, 2, 3, 4, 5].map((rating) => {
          const y = getWellnessY(rating);
          return (
            <text
              key={rating}
        ...
        {/* 5. Glowing Trend Lines */}
        {/* Adherence Curve (Solid primary line) */}
        ...
        {/* Energy Curve (Dashed green line) */}
          ...
          >
            ...
            {currentHoveredData.energy !== null && (
              <div className="flex items-center gap-2 justify-between">
                ...
                <span className="text-[11px] font-extrabold text-foreground">
                  {currentHoveredData.energy.toFixed(1)}/5
                </span>
              </div>
            )}
            ...
            {currentHoveredData.mood !== null && (
              <div className="flex items-center gap-2 justify-between">
                ...
                <span className="text-[11px] font-extrabold text-foreground">
                  {currentHoveredData.mood.toFixed(1)}/5
                </span>
              </div>
            )}
          </motion.div>
