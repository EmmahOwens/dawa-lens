...
async function reverseGeocode(
  ...
  try {
    ...
    console.warn(
      "Reverse geocoding failed, falling back to timezone heuristic:",
      err
    );
  }
  return null;
...
export function useGeolocation() {
  ...
  const requestLocation = useCallback(async () => {
    ...
    // On native: use cached country lookup (battery-efficient, 30-min TTL)
    if (Capacitor.isNativePlatform()) {
      try {
        ...
        console.warn(
          "[useGeolocation] NativeLocation failed, falling back to web path:",
          err
        );
        // Fall through to existing GPS + Nominatim path
      }
    }
